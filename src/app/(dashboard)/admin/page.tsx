...
import { Cohort, Application } from "@/interfaces";
...
export default function AdminDashboardPage() {
  ...
  const { data: todayData } = useQuery({
    queryKey: ["today-training-summary"],
    queryFn: async () => {
  ...
  const { data: cohortsData } = useQuery({
    queryKey: ["cohorts-capacity-admin"],
    queryFn: async () => {
  ...
  const { data: applicationsData } = useQuery({
    queryKey: ["applications-pending-admin"],
    queryFn: async () => {
      const res = await api.get("/applications?status=Submitted");
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Real-time central operational metrics across Adele training
              centres, multi-skill cohorts, and biometric attendance.
            </p>
          </div>
